...
function buildTitle(items: LocalComparisonItem[]): string {
...
// Column width constant for consistent horizontal scroll alignment
const COL_W = 220;

// ---------------------------------------------------------------------------
// Add-product overlay panel (PriceSpy-style)
// ---------------------------------------------------------------------------
...
function AddPanel({
  ...
  // Derive the subcategory scope from the first comparison item
  ...
  // Search scoped to the same productType so user can't mix categories
...
export default function ComparisonPage() {
  ...
  const title = buildTitle(comparisonItems);
            ...
            {comparisonItems.map((item) => (
              ...
              >
                ...
                <div className="p-3 text-center">
                  {/* Image */}
                  <Link to={`/product/pr/${item.product_id}`}>
